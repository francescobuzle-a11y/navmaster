...
start --es nm_dest "43.9360,12.4460" --es nm_label "'San Marino'" --es nm_profile camper --es nm_load 0.4 --ez nm_sim true
...
log
grep -E "route computed|scan:|criticalities|Valhalla ready|edges in|request=|FATAL|Exception" "$OUT/logcat.txt" | head -40 >> "$INFO"
echo done >> "$INFO"
